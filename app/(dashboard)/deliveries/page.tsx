...
function DeliveriesTable({ deliveries, activeStatus }: { deliveries: DeliveryWithDetails[], activeStatus: string }) {
  if (deliveries.length === 0) {
    return (
      <tr>
        <td colSpan={8} style={{ padding: '48px', textAlign: 'center', color: '#64748b' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
            <PackageMinus size={32} style={{ opacity: 0.3 }} />
            <span>No deliveries found. Create one to start tracking outgoing stock.</span>
          </div>
        </td>
      </tr>
    );
  ...
  return (
    <>
      {deliveries.map((delivery) => (
        <tr key={delivery.id} style={{ cursor: 'pointer' }}
          className="delivery-row">
          <td style={{ ...tdStyle, fontFamily: 'JetBrains Mono, monospace', color: '#60a5fa', fontSize: 12 }}>
            {delivery.reference}
          </td>
          <td style={{ ...tdStyle, fontWeight: 500 }}>{delivery.customer_name}</td>
          <td style={tdStyle}>
            <div style={{ color: '#e2e8f0' }}>{delivery.source_location_name}</div>
            <div style={{ fontSize: 11, color: '#64748b' }}>{delivery.source_warehouse_name}</div>
          </td>
          ...
          <td style={{ ...tdStyle }}>
            <Link href={`/deliveries/${delivery.id}`} style={{
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <h1 style={{ fontSize: 22, fontWeight: 700, color: '#f0f4ff', margin: 0 }}>Delivery Orders</h1>
            <span style={{
      ...
      <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid rgba(255,255,255,0.06)', paddingBottom: 0 }}>
        {statusTabs.map((tab) => (
          <Link
            key={tab.value}
            href={`/deliveries?status=${tab.value}`}
            style={{
              padding: '8px 16px',
              fontSize: 13,
      ...
      {/* Search */}
      <form style={{ display: 'flex', gap: 10 }} method="GET" action="/deliveries">
      ...
      <div style={{
        ...
        <style>{`.delivery-row:hover { background: rgba(37,99,235,0.05); }`}</style>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            <DeliveriesTable deliveries={deliveries} activeStatus={activeStatus} />
          </tbody>
        </table>
      </div>
...
export default async function DeliveriesPage({ searchParams }: DeliveriesPageProps) {
  return (
    <div style={{ padding: '24px', maxWidth: 1200, margin: '0 auto' }}>
      <Suspense fallback={
        <div style={{ color: '#64748b', padding: 48, textAlign: 'center' }}>Loading deliveries...</div>
      }>
